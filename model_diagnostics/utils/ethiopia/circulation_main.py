...
Produces inside base/model_diagnostics/output/ethiopia/YYYYMMDDTHH/:
    wind_850_YYYYMMDDTHH.png    — 2×3 850 hPa wind speed + quivers
    wind_200_YYYYMMDDTHH.png    — 2×3 200 hPa wind speed + quivers
    divcon_850_YYYYMMDDTHH.png  — 2×3 850 hPa convergence/divergence + quivers
    divcon_200_YYYYMMDDTHH.png  — 2×3 200 hPa convergence/divergence + quivers
...
CMAP_DIV = ListedColormap(_COLORS_DIV, name="divcon")
CMAP_DIV.set_under(_COLORS_DIV[0])
CMAP_DIV.set_over(_COLORS_DIV[-1])
NORM_DIV = BoundaryNorm(_LEVELS_DIV, CMAP_DIV.N)
...
def _divcon_figure(
    ...
    cbar_ax = fig.add_axes([0.92, 0.15, 0.015, 0.7])
    fig.colorbar(im, cax=cbar_ax, extend="both", ticks=_LEVELS_DIV).set_label(
