...
def forecast_onset_cal_NeuralGCM_GFS(ds_model, number, MWMean):
    ...
    ds_model_daily_np_Onset = ds_model_TS_daily_np.reshape(
        ds_model_TS_daily_np.shape[0] * ds_model_TS_daily_np.shape[1], 1
    )  # number of initializations * number of days
    ...
    dry = 5
    window = 10
    o1_model, o2_model, MWMean_model = onset_agro_bis(
        ds_model_daily_np_Onset, lseason, defdry, sw, wet, sd, dry, window
    )
    # print(o1_model)
    model_Onset = pd.DataFrame(o1_model, columns=["Onset_Days"])
    model_Onset["Date_Init"] = ds_model["time"].dt.date
    model_Onset["Onset_Date"] = model_Onset.apply(
        lambda x: (
            x.Date_Init + pd.Timedelta(days=(x.Onset_Days + 1))
            if pd.notnull(x["Onset_Days"]) and pd.notnull(x["Date_Init"])
            else np.nan
        ),
        axis=1,
    )
...
def NGCM_Prob_Calc_first(df_ngcm, lat, lon, df):
    ...
    for number in range(30):
        ...
        final_Onset[number] = (
            filtered_df  # Store filtered_df under the (fy, number) key pair
        )
    ...
    onset_cols = [
        col for col in all_ensemble_data.columns if col.startswith("Onset_Date_")
    ]
    all_ensemble_data[onset_cols] = all_ensemble_data[onset_cols].apply(
    ...
    stacked_data["Bin"] = stacked_data.apply(
        lambda row: get_week_bin(row["Onset_Date"], row["Date_Init"]), axis=1
    )
    bin_counts = (
        stacked_data.groupby("Date_Init")["Bin"].value_counts().unstack(fill_value=0)
    )
    ...
    df_melted["forecast_end_date"] = df_melted["Date_Init"] + pd.to_timedelta(
        df_melted["end_day_relative"], unit="D"
    )
    # Calculate the start date (end date - 7 days)
    df_melted["forecast_start_date"] = df_melted["forecast_end_date"] - pd.Timedelta(
        days=7
    )
...
def plot_precip(
    ...
    for _, meta in df_metadata.iterrows():
        ...
        df_daily = df_ts.pivot(
            index="datetime", columns="number", values="RAINFALL_Ini"
        )
        ...
        df_ts_AIFS["datetime"] = df_ts_AIFS["time"] + pd.to_timedelta(
            df_ts_AIFS["TIME"], unit="D"
        )
        ...
        group = df_prob.query("Date_Init==@init_date").sort_values(
            "forecast_start_date"
        )
        ...
        ax1.plot(
            dates1,
            df_daily.mean(axis=1),
            color="blue",
            lw=1.5,
            label="NGCM Ensemble Mean",
        )

        # data_AIFS  = [df_daily_AIFS.loc[d].values for d in dates1]
        ax1.plot(
            df_daily_AIFS.index,
            df_daily_AIFS["RAINFALL_Ini"],
            color="crimson",
            lw=1.5,
            label="AIFS",
        )
        ...
        for i in range(len(group) - 1):
            ...
            if s > max_sum:
                max_sum, bs, be = (
                    s,
                    group.forecast_start_date.iloc[i],
                    group.forecast_end_date.iloc[i + 1],
                )
        if bs:
            ax3.hlines(
                y=-0.05,
        ...
        fig.suptitle(
            f"Initialization {init_date.date()} — Lat={lat}, Lon={lon}",
            y=0.96,
            fontsize=11,
        )
    ...
    def NGCM_Prob_Calc(ds_ngcm, lat, lon, df_meta):
        ...
        MWMean = df_meta.loc[
            (df_meta.lat == lat) & (df_meta.lon == lon), "onset_thresh"
        ].iloc[0]

        all_ens = {
            n: forecast_onset_cal_NeuralGCM_GFS(grid, n, MWMean) for n in range(30)
        }
        ...
        for n, df_e in all_ens.items():
            ...
            tmp = tmp.set_index("Date_Init").rename(
                columns={"Onset_Date": f"Onset_Date_{n + 1}"}
            )
        ...
        df_long["Bin"] = df_long.apply(
            lambda r: get_week_bin(r["Onset_Date"], r["Date_Init"]), axis=1
        )

        counts = (
            df_long.groupby("Date_Init")["Bin"]
            .value_counts()
        ...
        dfm = probs.reset_index().melt(
            id_vars="Date_Init", var_name="forecast_week", value_name="probability"
        )
        dfm["end_date"] = dfm["Date_Init"] + pd.to_timedelta(
            dfm["forecast_week"].map(week_days), "D"
        )
    ...
    for ax, wk in zip(axes, WEEKS):
        ...
        agg["geometry"] = agg.apply(
            lambda r: box(r.lon - 1, r.lat - 1, r.lon + 1, r.lat + 1), axis=1
        )
    ...
    sm._A = []
    cbar = fig.colorbar(
        sm, ax=axes.tolist(), orientation="horizontal", fraction=0.03, pad=-0.2
    )
    cbar.set_label("Probability", fontsize=9)
    cbar.ax.tick_params(labelsize=8)

    plt.suptitle(
        "Monsoon Onset Probabilities by Week (2°×2° boxes)", y=0.98, fontsize=12
    )
