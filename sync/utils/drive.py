...
def authenticate():
    ...
    if not credentials_path.exists():
        raise FileNotFoundError(f"Credentials file '{credentials_path}' not found. Please download it from Google Cloud Console.")
        return None
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            try:
                ...
                logging.error(f"Error refreshing token: {e}. Deleting invalid token file.")
                # If refresh fails, delete token and trigger re-authentication
                try:
                    token_path.unlink(missing_ok=True)
                except OSError as unlink_err:
                    logging.error(f"Error deleting token file '{token_path}': {unlink_err}")
...
def get_or_create_folder_id(service, folder_name, parent_id='root'):
    ...
    try:
        ...
        if folders:
            ...
            logging.info(f"Folder '{folder_name}' not found in parent '{parent_id}', creating...")
            file_metadata = {
                'name': folder_name,
        ...
        if error.resp.status == 403:
             logging.error(f"Permission error accessing/creating folder '{folder_name}' in parent '{parent_id}'. Check Drive permissions.")
        elif error.resp.status == 404:
             logging.error(f"Parent folder ID '{parent_id}' not found when searching for/creating '{folder_name}'.")
        else:
            logging.error(f"An HTTP error occurred finding/creating folder '{folder_name}': {error}")
        return None
    except Exception as e:
...
def get_folder_id_by_path(service, path_string):
    ...
    for part in path_parts:
        logging.info(f"Searching for folder '{part}' in parent '{current_parent_id}' (Path: '{current_resolved_path}')")
        folder_id = get_or_create_folder_id(service, part, current_parent_id)
        if folder_id is None:
            logging.error(f"Failed to get or create folder part: '{part}' in path '{path_string}'")
...
def check_file_exists(service, file_name, drive_folder_id):
    ...
    try:
        ...
        query = (f"name='{safe_file_name}' and "
                 f"'{drive_folder_id}' in parents and "
                 f"trashed=false and "
                 f"mimeType!='application/vnd.google-apps.folder'") # Ensure it's not a folder
        ...
        logging.error(f"Error checking existence for file '{file_name}' in folder '{drive_folder_id}': {error}")
        return False # Assume not found on error to potentially allow upload attempt
    except Exception as e:
        logging.error(f"Unexpected error checking file existence for '{file_name}': {e}")
        return False
...
def upload_file(service, local_file_path, drive_folder_id):
    ...
    logging.info(f"Checking if '{file_name}' exists in Drive folder ID '{drive_folder_id}'...")
    if check_file_exists(service, file_name, drive_folder_id):
        logging.info(f"File '{file_name}' already exists in Drive folder '{drive_folder_id}'. Skipping upload.")
    ...
    try:
        ...
        file = service.files().create(body=file_metadata,
                                      ...
                                      fields='id, name',
                                      supportsAllDrives=True).execute()
        logging.info(f"Uploaded '{file_name}' to Drive folder ID '{drive_folder_id}' with ID: {file.get('id')}")
        return file.get('id')
    except HttpError as error:
        logging.error(f"Error uploading file '{file_name}' to Drive folder '{drive_folder_id}': {error}")
...
def upload_directory_recursive(service, local_dir_path, drive_parent_folder_id):
    ...
    if not local_path.is_dir():
        logging.error(f"Error: Local directory not found or is not a directory: {local_path}")
        return

    logging.info(f"Processing directory '{local_path}' for upload to Drive folder ID '{drive_parent_folder_id}'")
    ...
    for root, dirs, files in os.walk(str(local_path)):
        ...
        if relative_path != Path('.'): # If not the top-level dir passed initially
            ...
            for part in relative_path.parts:
                 logging.info(f"Creating/finding Drive subfolder '{part}' for path '{relative_path}'")
                 folder_id = get_or_create_folder_id(service, part, parent_drive_id_for_current)
                 if folder_id is None:
                      logging.error(f"Failed to create/find Drive subfolder '{part}' for path '{relative_path}'. Skipping contents.")
                 ...
                 parent_drive_id_for_current = folder_id # Update parent for the next part
...
def drive_sync(date, cluster): # Added cluster parameter with default
    ...
    if drive_service:
        ...
        cluster_base_drive_folder_id = get_folder_id_by_path(drive_service, DRIVE_CLUSTER_BASE_PATH)

        if cluster_base_drive_folder_id:
            # 2. Create or get the date-specific folder inside the cluster base path
            logging.info(f"Ensuring date folder '{date}' exists under '{DRIVE_CLUSTER_BASE_PATH}'...")
            date_folder_id = get_or_create_folder_id(drive_service, date, cluster_base_drive_folder_id)
            ...
            if date_folder_id:
                ...
                logging.info(f"Ensuring Drive folder '{aifs_target_folder_name}' exists...")
                aifs_drive_folder_id = get_or_create_folder_id(drive_service, aifs_target_folder_name, date_folder_id)
                if aifs_drive_folder_id:
                    logging.info(f"Uploading AIFS files to folder ID: {aifs_drive_folder_id}")
                    for file_path in AIFS_FILES_TO_UPLOAD[aifs_target_folder_name]:
                        upload_file(drive_service, file_path, aifs_drive_folder_id)
                else:
                    logging.error(f"ERROR: Could not create/find folder '{aifs_target_folder_name}', skipping AIFS uploads.")

                # 4. Upload NeuralGCM files
                logging.info("Processing NeuralGCM files...")
                ngcm_target_folder_name = list(NGCM_FILES_TO_UPLOAD.keys())[0]
                logging.info(f"Ensuring Drive folder '{ngcm_target_folder_name}' exists...")
                ngcm_drive_folder_id = get_or_create_folder_id(drive_service, ngcm_target_folder_name, date_folder_id)
                if ngcm_drive_folder_id:
                    logging.info(f"Uploading NeuralGCM files to folder ID: {ngcm_drive_folder_id}")
                    for file_path in NGCM_FILES_TO_UPLOAD[ngcm_target_folder_name]:
                        upload_file(drive_service, file_path, ngcm_drive_folder_id)
                else:
                    logging.error(f"ERROR: Could not create/find folder '{ngcm_target_folder_name}', skipping NeuralGCM uploads.")
                ...
                logging.info(f"Ensuring Drive folder '{blend_target_folder_name}' exists...")
                blend_drive_folder_id = get_or_create_folder_id(drive_service, blend_target_folder_name, date_folder_id)

                if blend_drive_folder_id:
                    if blend_local_source_dir.is_dir():
                        logging.info(f"Uploading contents of '{blend_local_source_dir}' to Drive folder ID: {blend_drive_folder_id}")
                        # Pass the starting local path and the target Drive folder ID
                        upload_directory_recursive(drive_service, blend_local_source_dir, blend_drive_folder_id)
                    else:
                        logging.error(f"ERROR: Local blend directory '{blend_local_source_dir}' not found or is not a directory. Skipping blend upload.")
                else:
                    logging.error(f"ERROR: Could not create/find folder '{blend_target_folder_name}', skipping blend upload.")

                logging.info("Sync process finished.")
            else:
                logging.error(f"CRITICAL: Could not create or find the main date folder '{date}' under '{DRIVE_CLUSTER_BASE_PATH}'. Aborting.")
        else:
            logging.error(f"CRITICAL: Could not create or find the cluster base path '{DRIVE_CLUSTER_BASE_PATH}'. Aborting.")
    else:
...
def drive_sync_IMERG(date, cluster): # Added cluster parameter with default
    ...
    try:
        ...
        if not (base / "IMERG").exists(): # Basic sanity check
             logging.warning("Base path might be incorrect. Expected IMERG folder not found.")
    ...
    if drive_service:
        ...
        if cluster_base_drive_folder_id:
            ...
            if date_folder_id:
                ...
                logging.info(f"Ensuring Drive folder '{IMERG_target_folder_name}' exists...")
                IMERG_drive_folder_id = get_or_create_folder_id(drive_service, IMERG_target_folder_name, date_folder_id)

                if IMERG_drive_folder_id:
                    if IMERG_local_source_dir.is_dir():
                        logging.info(f"Uploading contents of '{IMERG_local_source_dir}' to Drive folder ID: {IMERG_drive_folder_id}")
                        # Pass the starting local path and the target Drive folder ID
                        upload_directory_recursive(drive_service, IMERG_local_source_dir, IMERG_drive_folder_id)
                    else:
                        logging.error(f"ERROR: Local IMERG directory '{IMERG_local_source_dir}' not found or is not a directory. Skipping IMERG upload.")
                else:
                    logging.error(f"ERROR: Could not create/find folder '{IMERG_target_folder_name}', skipping IMERG upload.")
