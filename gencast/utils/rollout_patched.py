...
def dataset_allgather(dataset: xarray.Dataset) -> xarray.Dataset:
  """Allgathers dataset variables along 'sample' dimension in multi-process setups."""
  if dataset is None:
    return None

  def gather_variable(var: xarray.Variable) -> xarray.Variable:
    if "sample" in var.dims:
      sample_axis = var.dims.index("sample")
      assert sample_axis == 0, "sample dimension must be at axis 0 for allgather"
      
      # Perform process_allgather on the underlying numpy array
      gathered_data = multihost_utils.process_allgather(var.data, tiled=True)
  ...
  data_vars = {name: gather_variable(var) for name, var in dataset.data_vars.variables.items()}
  coords = {name: coord.variable for name, coord in dataset.coords.items()}
  
  # Update 'sample' coordinate to be global
  global_sample_size = jax.process_count() * dataset.dims["sample"]
  coords["sample"] = xarray.Variable(
      dims=("sample",),
      data=np.arange(global_sample_size)
  )
  
  return xarray.Dataset(data_vars, coords=coords, attrs=dataset.attrs)
...
def chunked_prediction_generator_multiple_runs(
  ...
  if pmap_devices is not None:
    ...
    # Use local devices for replication in multi-process configurations
    if jax.process_count() > 1:
      local_devices = jax.local_devices()
    else:
      local_devices = pmap_devices
    ...
    for i in range(0, num_samples, len(pmap_devices)):
      # Calculate local slice for the current process
      if jax.process_count() > 1:
        process_index = jax.process_index()
        local_device_count = len(local_devices)
        local_start = i + process_index * local_device_count
        local_end = local_start + local_device_count
        sample_idx = slice(local_start, local_end)
      else:
        sample_idx = slice(i, i + len(pmap_devices))

      logging.info(
          "Process %d/%d: local samples %s out of global %s (step %d/%d)",
          jax.process_index(),
          jax.process_count(),
          sample_idx,
          num_samples,
      ...
      sample_group_rngs = _device_put_sharded(
          rngs[sample_idx], local_devices, "sample")
      ...
      for prediction_chunk in chunked_prediction_generator(
          ...
          pmap_devices=local_devices,
          replica_axis="sample",
          **chunked_prediction_kwargs,
      ):
        if jax.process_count() > 1:
          prediction_chunk = dataset_allgather(prediction_chunk)
        else:
