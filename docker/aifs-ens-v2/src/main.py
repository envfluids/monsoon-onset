"""
Monsoon AIFS ensemble v2 GCS wrapper.

Runs the AIFS_ENS_v2 science entrypoint and per-region post-processing. Cloud
paths use the exact versioned model name.
"""
...
AIFS_ENS_MIRROR_WORKERS = os.environ.get("AIFS_ENS_MIRROR_WORKERS", "16")
...
def main(date, model, regions, common_bucket, region_buckets, upload_full_field):
    ...
    logger.info(
        "AIFS ENS v2 shim: model=%s date=%s regions=%s upload_full_field=%s",
        model,
        date,
        regions,
        upload_full_field,
    ...
    zarr_mirror_target = _zarr_mirror_target(date, model, upload_full_field)
    _run_inference(date, model, zarr_mirror_target)
    if zarr_mirror_target is not None:
        logger.info("AIFS ENS v2 full-field Zarr was mirrored through GCS FUSE")
...
def _zarr_mirror_target(aifs_date: str, model: str, upload_full_field: bool) -> Path | None:
    if not upload_full_field:
        return None
    if not COMMON_BUCKET_MOUNT.is_dir():
        raise RuntimeError(
            "AIFS ENS v2 full-field upload requires the GCS FUSE mount at "
            f"{COMMON_BUCKET_MOUNT}, but it is unavailable."
        )
    target = COMMON_BUCKET_MOUNT / "full_field" / model / aifs_date / f"init_{aifs_date}.zarr"
    logger.info("AIFS ENS v2 full-field Zarr mirror target: %s", target)
    return target
...
def _upload_full_field(aifs_date: str, model: str, common_bucket: str) -> None:
    ...
    if raw_path.is_dir():
        upload_directory(common_bucket, raw_path, target)
