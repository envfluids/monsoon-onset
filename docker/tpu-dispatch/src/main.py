...
GENCAST_OUTPUT_DIR="${COMMON_BUCKET_MOUNT}/full_field/gencast/${DATE}"
GENCAST_ZARR_MIRROR_WORKERS=16
GENCAST_ASYNC_WRITER_MAX_PENDING=8
GENCAST_GCSFUSE_BUCKET="$COMMON_BUCKET"
GENCAST_GCSFUSE_MOUNT="$COMMON_BUCKET_MOUNT"
GENCAST_ENABLE_JAX_COMPILATION_CACHE=true
JAX_COMPILATION_CACHE_DIR="${COMMON_BUCKET_MOUNT}/jax-cache/gencast/v5p-64"
...
docker run --rm --privileged --net=host --name "monsoon-${WORKLOAD_NAME}-${DATE}" \
  ...
  -e GENCAST_OUTPUT_DIR="$GENCAST_OUTPUT_DIR" \
  -e GENCAST_ZARR_MIRROR_WORKERS="$GENCAST_ZARR_MIRROR_WORKERS" \
  -e GENCAST_ASYNC_WRITER_MAX_PENDING="$GENCAST_ASYNC_WRITER_MAX_PENDING" \
  -e GENCAST_GCSFUSE_BUCKET="$GENCAST_GCSFUSE_BUCKET" \
  -e GENCAST_GCSFUSE_MOUNT="$GENCAST_GCSFUSE_MOUNT" \
  -e GENCAST_ENABLE_JAX_COMPILATION_CACHE="$GENCAST_ENABLE_JAX_COMPILATION_CACHE" \
  -e JAX_COMPILATION_CACHE_DIR="$JAX_COMPILATION_CACHE_DIR" \
  -e JAX_PERSISTENT_CACHE_MIN_COMPILE_TIME_SECS="$JAX_PERSISTENT_CACHE_MIN_COMPILE_TIME_SECS" \
