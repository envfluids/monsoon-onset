...
def forecast_onset_cal_IFSS2S(ds_model, number, MWMean):
...
def IFSS2S_Prob_Calc(df_IFS, lat, lon, df):
    ...
    for number in range(101):
        filtered_df = forecast_onset_cal_IFSS2S(df_IFS_grid, number, MWMean)
    ...
    for i in range(101):  # Ensemble members 0 to 29
    ...
    #bin_prob.to_csv(f"IFS_Probabilites_{lat}_{lon}.csv")
...
def plot_IFSS2S_Ensembles_and_Probabilities(df_IFS, df_AIFS, df_meta, folder_out):
    # 0) Metadata
    df_metadata = df_meta.copy()

    ts64 = df_IFS['time'].isel(time=0).item()
    ...
    for _, meta in df_metadata.iterrows():
        ...
        da = df_IFS.sel(lat=lat, lon=lon).rename({"day":"TIME"})
        ...
        df_prob = IFSS2S_Prob_Calc(df_IFS, lat, lon, df_metadata)
        ...
        fig.savefig(folder_out / outfn, dpi=300, bbox_inches="tight")
        plt.close(fig)
        print(f"Saved {outfn}")
...
# --- Shared binning function (from Code 1) ---
    ...
    # --- PARAMETERS ---
def plot_IFSS2S_Probabilities(df_IFS, df_meta, folder_out):
    ...
    def IFS_Prob_Calc(ds_IFS, lat, lon, df_meta):
        grid = ds_IFS.sel(lat=lat, lon=lon)
        MWMean    = df_meta.loc[(df_meta.lat==lat)&(df_meta.lon==lon),'onset_thresh'].iloc[0]
        
        all_ens = {
        n: forecast_onset_cal_IFSS2S(grid, n, MWMean)
        #for n in range(30)
        for n in range(101)
        }
    ...
    for _, m in df_metadata.iterrows():
        dfp = IFS_Prob_Calc(df_IFS, m.lat, m.lon, df_metadata)
    ...
    plt.suptitle('IFS-S2S Monsoon Onset Probabilities by Week (2°×2° boxes)', y=0.98, fontsize=12)
    plt.tight_layout(rect=[0,0,1,0.95])

    if not os.path.exists(folder_out):
        os.makedirs(folder_out)
    fig.savefig(folder_out/OUTPUT_FILE, dpi=300, bbox_inches='tight')
    #plt.close(fig)
...
def plot_all(date_f):
    ...
    onsets_2024_path = base.parent / "blend" / "data" / "support" / "onsets_2024.csv"
    ...
    IFS_DATA_PATH = base / "raw" / "netcdf" / f"ifs_s2s_init_{date_f}.nc"
    ...
    plot_IFSS2S_Probabilities(df_IFS, df_meta, out_folder)
    print(f"Plots saved to {out_folder}")

if __name__ == "__main__":
    plot_all("20250612T00")
