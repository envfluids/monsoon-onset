...
# from cdo import *
import glob
from concurrent.futures import ProcessPoolExecutor, as_completed # Import necessary components
...
def process_member(member, date):
    ...
    ds = xr.open_zarr(f"../raw/output/{date}/member_{member}.zarr").rename({'precipitation_cumulative_mean':'tp', 'latitude':'lat', 'longitude':'lon'}).isel(ensemble=0, surface=0)
    ds = preprocess(ds)
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=n_members) as executor:
        print(f"Submitting tasks for {n_members} members using up to {n_members} workers...")
        ...
        for i, future in enumerate(as_completed(futures), 1):
            try:
                result = future.result() # Get the return value from the function (file path or None)
                print(f"  Completed task {i}/{n_members} for {date}. Success: {result is not None}")
                if result:
                    processed_files.append(result)
            except Exception as e:
                # Catch exceptions raised *during* the execution of the task in the worker process
                print(f"ERROR retrieving result for a task in {date}: {type(e).__name__} - {e}")
