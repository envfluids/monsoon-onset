...
def get_latest_available_cycle(pattern, num_cycles_to_check, cycles, timeout):
    ...
    for i in range(num_cycles_to_check):
        ...
        approx_cycle_time = now_utc - datetime.timedelta(
            hours=(now_utc.hour % 6) + (i * 6)
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logging.debug(
                    f"Not found: {url}"
                )  # Use debug for non-critical 'not found'
            else:
                logging.warning(
        ...
        with requests.get(url, stream=True, timeout=timeout) as response:
            ...
            with (
                ...
                tqdm(
                    ...
                    unit_scale=True,
                    desc=filename,
                    leave=False,  # leave=False cleans up bar on completion
                ) as bar,
            ):
