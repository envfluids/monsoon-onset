...
def main():
    ...
    date = args.date
    ngcm_out = xr.open_mfdataset(f"../raw/output/{date}/*.zarr", engine='zarr', preprocess=preprocess)
    ngcm_out = ngcm_out.rename({'latitude':'lat', 'longitude':'lon'})
    ngcm_out = ngcm_out[['v_component_of_wind','u_component_of_wind']].sel(level=850).drop_vars('level')
