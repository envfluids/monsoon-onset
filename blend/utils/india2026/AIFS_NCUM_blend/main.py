...
def check_input_files(date: str, base: Path) -> bool:
    """
    Check if AIFS and NCUM forecast files exist.
    Returns True if all required files exist, False otherwise.
    """
    aifs_file = base / AIFS_CONFIG.file_template.format(date=date)
    ncum_file = base / NCUM_CONFIG.file_template.format(date=date)

    logging.info(f"Checking AIFS file: {aifs_file}")
    if not aifs_file.exists():
        logging.error(f"AIFS file not found: {aifs_file}")
        return False
    logging.info("AIFS file found")

    logging.info(f"Checking NCUM file: {ncum_file}")
    if not ncum_file.exists():
        logging.error(f"NCUM file not found: {ncum_file}")
        return False
    logging.info("NCUM file found")

    return True


def get_data(date: str, base: Path, source: str | None = None):
    """
    ...
    for cfg in ACTIVE_MODELS:
        if not cfg.file_template:
            logging.warning(f"Model '{cfg.label}' has no file_template; skipping.")
            continue
...
def main():
    ...
    if not check_input_files(date, base):
