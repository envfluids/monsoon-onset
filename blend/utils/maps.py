...
def make_maps(summary, date):
    ...
    preds_df["Climatology_p_12"] = (
        preds_df["Climatology_p_1"] + preds_df["Climatology_p_2"]
    )
    preds_df["Climatology_p_34"] = (
        preds_df["Climatology_p_3"] + preds_df["Climatology_p_4"]
    )
    ...
    for tag in ["1", "2", "3", "4", "12", "34"]:
        ...
        preds_df[f"outline_{tag}"] = np.where(
            clim >= fcst + 0.10, "red", np.where(fcst >= clim + 0.10, "green", "black")
        )
...
def make_extra_maps(summary, date):
    ...
    preds_df["Climatology_p_later"] = 1 - preds_df[
    ...
    for t, grp in preds_df.groupby("time"):
        ds = t.strftime("%Y-%m-%d")
        week_titles = {
            i: f"{(t + timedelta(days=(i - 1) * 7 + 1)).strftime('%m/%d/%Y')} - {(t + timedelta(days=(i - 1) * 7 + 7)).strftime('%m/%d/%Y')}"
            for i in range(1, 5)
        }
            ...
            for _, r in grp.iterrows():
                ...
                ax.add_patch(
                    Rectangle(
                        ...
                        w,
                        h,
                        facecolor=prob_cmap(prob_norm(v)),
                        edgecolor="none",
                        zorder=1,
                    )
                )
        ...
        outline_handles = [
            Patch(
                ...
                edgecolor="red",
                linewidth=3,
                label="≥10% lower than climatology",
            ),
            Patch(
                facecolor="none",
                edgecolor="green",
                linewidth=3,
                label="≥10% higher than climatology",
            ),
        ]
        fig.legend(
            handles=outline_handles, loc="lower right", bbox_to_anchor=(0.98, 0.02)
        )
    ...
    for t, grp in preds_df.groupby("time"):
        ...
        for k in period_order:
            sd, ed = pdays[k]
            lbl = f"{(t + timedelta(days=sd)).strftime('%m/%d/%Y')}" + (
                f" - {(t + timedelta(days=ed)).strftime('%m/%d/%Y')}" if ed else "+"
            )
            handles.append(
                Patch(facecolor=period_colors[k], edgecolor="black", label=lbl)
            )
        handles.append(
            Patch(facecolor=period_colors["none"], edgecolor="black", label="Uncertain")
        )
        fig.legend(
            handles=handles,
            title="Max Period",
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/map_bars_{ds}.png", dpi=150)
        plt.close(fig)
    ...
    for t, grp in preds_df.groupby("time"):
        ...
        for _, r in grp.iterrows():
            small_vals = [r[f"Forecast_p_{i}"] for i in range(1, 5)] + [
                r["Forecast_p_later"]
            ]
            ...
            if r["Forecast_p_later"] >= 0.5:
                ...
                two_sums = [
                    small_vals[0] + small_vals[1],
                    small_vals[1] + small_vals[2],
                    small_vals[2] + small_vals[3],
                    small_vals[3] + small_vals[4],
                ]
                keys = ["weeks12", "weeks23", "weeks34", "weeks4later"]
            ...
            def make_color(indices):
                return period_colors.get(
                    interval_map[indices[0]][0], period_colors["none"]
                )
