...
def make_maps(summary, output_path, mok=False):
    ...
    for t, grp in preds_df.groupby('time'):
        ...
        legend_handles = [Patch(facecolor='none', edgecolor=c, linewidth=3, label=la) for c, la in [('red','≥10% lower than climatology'),('green','≥10% higher than climatology')]]
    ...
    for t, grp in preds_df.groupby('time'):
        ...
        ax2.set_yticks(np.arange(18, 32+1, 2))
        ax2.set_xticks(np.arange(76, 88+1, 2))
        ...
        for name, centers in regions.items():
            ...
            lat_min_r=min(lats)-1
            lat_max_r=max(lats)+1
            lon_min_r=min(lons)-1
            lon_max_r=max(lons)+1
