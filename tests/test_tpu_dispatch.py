...
class TpuDispatchTest(unittest.TestCase):
    ...
    def test_startup_script_streams_and_uploads_node_logs(self):
        ...
        self.assertIn("-e GENCAST_OUTPUT_DIR=\"$GENCAST_OUTPUT_DIR\"", script)
        self.assertIn("GENCAST_OUTPUT_DIR=\"${COMMON_BUCKET_MOUNT}/full_field/gencast/${DATE}\"", script)
        self.assertNotIn("-e GENCAST_ZARR_MIRROR_TARGET", script)
        self.assertIn("-e GENCAST_ASYNC_WRITER_MAX_PENDING=\"$GENCAST_ASYNC_WRITER_MAX_PENDING\"", script)
        self.assertIn("GENCAST_ASYNC_WRITER_MAX_PENDING=8", script)
        self.assertIn("-e JAX_COMPILATION_CACHE_DIR=\"$JAX_COMPILATION_CACHE_DIR\"", script)
        self.assertIn("jax-cache/gencast/v5p-64", script)
