...
def get_IMERG(IMERG_data_dir, date_f):
    ...
    dat_ap = (
        dat.sel(lat=slice(7, 39), lon=slice(67, 101))
        .coarsen(lat=20, lon=20, boundary="trim")
        .mean()
    )
...
def get_model_data(base, date_f):
    ...
    ds_D = xr.Dataset(
        data_vars={"tp_IMERG": (("lat", "lon"), dat_ap.values.astype(np.float32))},
        coords={
...
def plot_IMERG_model_bias(PATHS, ds_D, For_date_times):
    ...
    df_meta = pd.read_csv(ONSET_CSV)[["lat", "lon", "onset_thresh"]].drop_duplicates(
        subset=["lat", "lon"]
    )
    ...
    for _, row in df_meta.iterrows():
        mask2d |= np.isclose(lat2d, row.lat) & np.isclose(lon2d, row.lon)
    ...
    for ax, (var, cmap, leg) in zip(axes, vars_cmaps):
        ...
        highlight_gdf.plot(
            ax=ax, facecolor="none", edgecolor="magenta", linewidth=1, zorder=3
        )

        # add horizontal colorbar
        cbar = fig.colorbar(
            im, ax=ax, orientation="horizontal", fraction=0.046, pad=0.08
        )
    ...
    for ax, (var, cmap, title), da_masked in zip(axes, vars_info, masked_arrays):
        ...
        im = da_masked.plot.pcolormesh(
            ax=ax, cmap=cmap, norm=norm, add_colorbar=False, zorder=1
        )
...
def main():
    ...
    logging.info("Plotting model bias")
    ...
    logging.info("Finished plotting model bias")
