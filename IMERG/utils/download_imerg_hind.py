...
def get_data(target_date):
    ...
    Args:
        target_date (datetime): The date for which to check and download data.
    ...
    try:
        ...
        if Path.cwd().name == "utils":
            ...
            base_dir = Path.cwd()
        logging.warning(
            f"Warning: __file__ not defined. Assuming base directory: {base_dir}"
        )
    ...
    try:
        spider_result = subprocess.run(
            spider_command, capture_output=True, text=True, check=False, timeout=60
        )
        ...
        if spider_result.returncode == 0:
            ...
            if local_file_path.exists():
                logging.info(
                    f"File {filename} already exists locally. No download needed."
                )
                ...
                download_command = [
                    ...
                    url,
                    "-P",
                    str(raw_dir),  # Download to the raw_dir directory
                ]
                ...
                if download_result.returncode == 0:
                    ...
                    if (raw_dir / filename).exists():
                        ...
                        logging.warning(
                            f"Downloaded file not found as {filename}. Checking raw_dir for any files with date string."
                        )
                        ...
                        downloaded_files = list(
                            raw_dir.glob(f"*{date_str_compact}*.nc4")
                        )
                        if downloaded_files:
                            actual_file = downloaded_files[0]
                            logging.info(
                                f"INFO: Found potential match: {actual_file.name}. Assuming this is the downloaded file."
                            )
                            ...
                            logging.error(
                                f"Downloaded file not found as {filename} and no other files with date string found."
                            )
                            return None
                else:
                    logging.error(
                        f"Failed to download {filename}. wget exit code: {download_result.returncode}"
                    )
            ...
            logging.warning(
                f"      wget --spider exit code: {spider_result.returncode}"
            )
            # if spider_result.stderr:
            # logging.warning(f"      STDERR: {spider_result.stderr.strip()}")
            if (
                "ERROR 401" in spider_result.stderr
                or "Authentication R" in spider_result.stderr
            ):
                logging.error(
                    "CRITICAL: Authentication error with wget. Ensure .urs_cookies is valid, accessible, and not expired."
                )
        ...
        logging.error(
            "CRITICAL: `wget` command not found. Please ensure wget is installed and in your system's PATH."
        )
        return None
        ...
        logging.error(
            f"CRITICAL: An unexpected error occurred during processing for {date_str_compact}: {e}"
        )
...
if __name__ == "__main__":
    ...
    while start_date <= end_date:
        downloaded_date = get_data(start_date)
        if downloaded_date:
